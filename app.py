"""CyberData Analytics — works on Streamlit Cloud and locally."""
...
def _build_demo_warehouse() -> duckdb.DuckDBPyConnection:
    """Synthetic in-memory warehouse for Streamlit Cloud / demo use."""
    con = duckdb.connect(":memory:")
    rng = np.random.default_rng(42)
    dates = [date(2024, 1, 1) + timedelta(days=i) for i in range(30)]
    n30 = 30

    def make(tbl: str, df: pd.DataFrame):
        view = f"_v_{tbl}"
        con.register(view, df)
        con.execute(f"CREATE TABLE {tbl} AS SELECT * FROM {view}")
    ...
    make("mart_kpi_daily", pd.DataFrame({
        ...
        "total_bytes":         rng.integers(100_000_000, 5_000_000_000, n30).tolist(),
    ...
    hosts = [f"WIN{i:03d}" for i in range(1,11)] + [f"ubuntu{i:02d}" for i in range(1,6)]
    n_ha = len(hosts) * 24
    ...
    make("mart_top_talkers", pd.DataFrame({
        ...
        "total_bytes":         rng.integers(1_000_000, 1_000_000_000, n30).tolist(),
    ...
    make("mart_ids_alerts", pd.DataFrame({
        "event_date":     dates,
        "signature":      rng.choice(["ET SCAN Port Scan","ET MALWARE Trojan","ET EXPLOIT Buffer Overflow","ET POLICY Suspicious UA"], n30).tolist(),
...
def get_db_connection():
    ...
    # ── Streamlit Cloud: build demo warehouse in-memory ───────────────────
...
with st.sidebar:
    ...
    is_demo = not os.path.exists(__import__("config").DB_PATH)
    st.caption(f"{'🟡 Demo data' if is_demo else '🟢 Live warehouse'}")
...
if page == "🏠 Executive Summary":
    ...
    with tab2:
        ...
        if not tk.empty:
            fig=px.scatter(tk,x="connection_count",y="unique_destinations",size="total_bytes",color="unique_dest_ports",hover_data=["src_ip"],title="Top Talkers",color_continuous_scale="Reds",size_max=50)
    ...
    with tab3:
        ...
        if not sc.empty:
            fig=px.bar(sc.head(15),x="src_ip",y="ports",color="scan_risk_level",color_discrete_map={"HIGH":"#f85149","MEDIUM":"#ffa657","LOW":"#3fb950"},title="Port Scan Suspects")
    ...
    with tab3:
        ...
        if not ah.empty:
            fig=px.density_heatmap(ah,x="login_hour",y="username",z="cnt",title="After-Hours Logins",color_continuous_scale="Reds")
    ...
    if not atk.empty:
        ...
        with c2:
            fig=px.scatter(atk,x="duration_minutes",y="hosts_affected",size="event_count",color="attack_type",title="Duration vs Hosts Affected",color_discrete_sequence=px.colors.qualitative.Bold,hover_data=["attack_type","event_count"])
    ...
    if not tl.empty:
        fig=px.scatter(tl,x="event_time",y="attack_type",color="mitre_technique",symbol="hostname",title="Attack Event Timeline",hover_data=["hostname","mitre_technique"])
