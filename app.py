...
def _build_demo_warehouse():
    ...
    Uses con.register() to avoid ALL DuckDB reserved-word conflicts."""
    ...
    # fact_events
    n = 2000
    atk_raw = rng.choice(["apt","ransomware","data_exfil","cred_theft","","",""], n).tolist()
    mit_raw = rng.choice(["T1059.001","T1021","T1041","T1003","T1566","",""], n).tolist()
    make("fact_events", pd.DataFrame({
        "event_time":        pd.date_range("2024-01-01", periods=n, freq="21min"),
        "dataset":           rng.choice(["zeek.connection","windows.security","suricata.alert","firewall.iptables"], n).tolist(),
        "event_kind":        rng.choice(["event","alert"], n).tolist(),
        "event_action":      rng.choice(["Successful Logon","Failed Logon","DROP","ACCEPT"], n).tolist(),
        "event_category":    rng.choice(["network","authentication","process","web"], n).tolist(),
        "src_ip":            [f"192.168.1.{int(rng.integers(1,254))}" for _ in range(n)],
        "dst_ip":            [f"10.0.{int(rng.integers(0,255))}.{int(rng.integers(1,254))}" for _ in range(n)],
        "src_port":          [str(int(x)) for x in rng.integers(1024, 65535, n)],
        "dst_port":          rng.choice(["80","443","22","53","3389"], n).tolist(),
        "protocol":          rng.choice(["tcp","udp","icmp"], n).tolist(),
        "transport":         rng.choice(["tcp","udp"], n).tolist(),
        "direction":         rng.choice(["inbound","outbound"], n).tolist(),
        "bytes_transferred": rng.integers(64, 65535, n).tolist(),
        "hostname":          [f"WIN{int(rng.integers(1,30)):03d}" if rng.random()<0.5 else f"ubuntu{int(rng.integers(1,20)):02d}" for _ in range(n)],
        "username":          rng.choice(["alice","bob","charlie","admin"], n).tolist(),
        "attack_type":       [x if x.strip() else None for x in atk_raw],
        "mitre_technique":   [x if x.strip() else None for x in mit_raw],
        "source_file":       "demo",
        "log_source":        rng.choice(["network","windows","suricata","firewall","sysmon"], n).tolist(),
    }))
    ...
    n_ha = len(hosts) * 24
    zscores = rng.normal(0, 1.5, n_ha).tolist()
    make("mart_host_anomaly", pd.DataFrame({
    ...
    make("mart_fw_blocked_sources", pd.DataFrame({
        "src_ip":         [f"{int(rng.integers(1,200))}.{int(rng.integers(0,255))}.{int(rng.integers(0,255))}.{int(rng.integers(1,254))}" for _ in range(20)],
        "block_count":    rng.integers(10, 1000, 20).tolist(),
        "targeted_ports": rng.integers(1, 30, 20).tolist(),
        "targeted_hosts": rng.integers(1, 10, 20).tolist(),
        "first_blocked":  pd.to_datetime(["2024-01-01"]*20),
