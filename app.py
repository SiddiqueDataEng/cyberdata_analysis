...
# Cloud: downloads warehouse from Google Drive | Local: uses local warehouse
# Fallback: pure-Python synthetic demo (no SQL strings)
...
try:
    ...
    _LOCAL_DB = os.path.exists(_CFG_DB)
except Exception:
    _CFG_DB = ""
    _LOCAL_DB = False
...
# Google Drive file ID for cyber_warehouse.duckdb
GDRIVE_FILE_ID = "19U1VQpspvFOhBwVokKS_wEc3-8_iWcZU"
CLOUD_DB_PATH  = "/tmp/cyber_warehouse.duckdb"
...
# ─────────────────────────────────────────────────────────────────────────────
# PURE-PYTHON FALLBACK DEMO (no SQL strings — only pandas + con.register)
# ─────────────────────────────────────────────────────────────────────────────
def _build_demo():
    """Build in-memory DuckDB entirely via pandas DataFrames — zero SQL strings."""
    ...
    def load(name: str, df: pd.DataFrame):
        """Register df then create table — no SQL template strings."""
        con.register(name, df)
        # Use DuckDB Python API to create table from registered view
        con.execute(f"CREATE TABLE {name} AS SELECT * FROM {name}")
        con.unregister(name)

    sc = rng.uniform(10, 90, n).tolist()

    load("mart_kpi_daily", pd.DataFrame({
        "event_date": dates,
    ...
    load("mart_risk_score_daily", pd.DataFrame({
        "event_date": dates,
        "ids_alerts": rng.integers(50, 800, n).tolist(),
        "attack_events": rng.integers(0, 50, n).tolist(),
    ...
    load("mart_attack_summary", pd.DataFrame({
    ...
    load("mart_mitre_coverage", pd.DataFrame({
    ...
    load("mart_host_anomaly", pd.DataFrame({
    ...
    load("mart_brute_force", pd.DataFrame({
    ...
    load("mart_top_talkers", pd.DataFrame({
    ...
    load("mart_ids_alerts", pd.DataFrame({
        "event_date": dates,
    ...
    load("mart_afterhours_logins", pd.DataFrame({
    ...
    load("mart_suspicious_processes", pd.DataFrame({
    ...
    load("mart_risky_powershell", pd.DataFrame({
    ...
    load("mart_firewall_summary", pd.DataFrame({
    ...
    load("mart_fw_blocked_sources", pd.DataFrame({
    ...
    load("mart_port_scan_suspects", pd.DataFrame({
    ...
    n_et = n * 24
    load("mart_external_traffic", pd.DataFrame({
    ...
    load("mart_protocol_dist", pd.DataFrame({
    ...
    load("mart_syslog_summary", pd.DataFrame({
        "event_date": [d for d in dates for _ in range(5)],
    ...
    load("mart_ssh_attempts", pd.DataFrame({
        "event_date": dates,
    ...
    load("mart_dns_summary", pd.DataFrame({
        "event_date": dates,
    ...
    load("mart_http_status", pd.DataFrame({
    ...
    n_wet = n * 24
    load("mart_web_error_trend", pd.DataFrame({
...
# ─────────────────────────────────────────────────────────────────────────────
# DB CONNECTION — tries 3 sources in order
# ─────────────────────────────────────────────────────────────────────────────
...
def get_con():
    ...
    if _LOCAL_DB:
        try:
            return duckdb.connect(_CFG_DB, read_only=True), "local"
        except Exception:
            pass

    # 2. Cloud: try to download from Google Drive
    if IS_CLOUD and not os.path.exists(CLOUD_DB_PATH):
        try:
            import gdown
            with st.spinner("Downloading warehouse from Google Drive..."):
                gdown.download(
                    f"https://drive.google.com/uc?id={GDRIVE_FILE_ID}",
                    CLOUD_DB_PATH, quiet=True
                )
        except Exception:
            pass
    ...
    if os.path.exists(CLOUD_DB_PATH):
        try:
            return duckdb.connect(CLOUD_DB_PATH, read_only=True), "gdrive"
        except Exception:
            pass

    # 3. Pure-Python fallback demo
    with st.spinner("Building demo warehouse..."):
        return _build_demo(), "demo"


_result = get_con()
CON, _DB_SOURCE = _result if isinstance(_result, tuple) else (_result, "unknown")
...
def qlim(sql: str) -> pd.DataFrame:
    q = sql.rstrip().rstrip(";")
    if _DB_SOURCE == "demo" and "LIMIT" not in q.upper():
        q += f" {ROW_LIMIT}"
    return qdf(q)
...
with st.sidebar:
    ...
    src_labels = {"local": "Live warehouse", "gdrive": "Cloud warehouse (Drive)", "demo": "Synthetic demo"}
    src_icons  = {"local": "green", "gdrive": "blue", "demo": "yellow"}
    st.caption(f"Data: {src_labels.get(_DB_SOURCE, _DB_SOURCE)}")
    st.divider()
...
if page == "Executive Summary":
    ...
    st.caption(f"Period: {d_start} to {d_end} | {src_labels.get(_DB_SOURCE, '')}")
    ...
    with cb:
        ...
        if not kc.empty:
            fig=px.bar(kc,x="cnt",y="kill_chain_phase",orientation="h",title="MITRE Kill Chain Phases",
...
# =============================================================================
    ...
    with c1:
        fig=go.Figure()
        for c2n,clr in [("ids_alerts","#f85149"),("auth_failures","#ffa657"),("fw_drops","#58a6ff"),("attack_events","#d2a8ff")]:
            fig.add_trace(go.Scatter(x=kpi_df["event_date"],y=kpi_df[c2n],name=c2n.replace("_"," ").title(),line=dict(color=clr,width=1.8)))
        fig.update_layout(title="Security Signal Trends",legend=dict(orientation="h",y=-0.2)); show(fig,h=300)
    with c2:
        fig=go.Figure()
        ...
        fig.update_layout(title="Login Success vs Failure",barmode="stack",legend=dict(orientation="h",y=-0.2)); show(fig,h=300)
    ...
    if not rsk.empty:
        ...
        fig.update_layout(title="Daily Risk Score",legend=dict(orientation="h",y=-0.2)); show(fig,h=260)
    st.markdown("#### Raw KPI Table")
    st.dataframe(kpi_df.drop(columns=["week","dow"],errors="ignore"),use_container_width=True,height=280)
...
# =============================================================================
    ...
    with t1:
        ...
        if not ext.empty:
            ...
            fig.update_layout(title="External Traffic Flow"); show(fig,h=260)
        proto=qdf(f"SELECT protocol,SUM(event_count) AS cnt FROM mart_protocol_dist WHERE {DR} GROUP BY 1 ORDER BY 2 DESC LIMIT 10")
        if not proto.empty:
            p1,p2=st.columns(2)
            with p1:
                fig=px.pie(proto,values="cnt",names="protocol",title="Protocol Distribution",hole=0.4,color_discrete_sequence=px.colors.qualitative.Set3); show(fig,h=240)
            with p2:
                fig=px.bar(proto,x="protocol",y="cnt",title="Events by Protocol",color="cnt",color_continuous_scale="Blues"); show(fig,h=240)
    with t2:
        ...
        if not tk.empty:
            ...
            show(fig,h=340); st.dataframe(tk,use_container_width=True)
    with t3:
        ...
        if not sc.empty:
            ...
            fig.update_layout(xaxis_tickangle=-45); show(fig,h=300)
    with t4:
        ...
        if not fw.empty:
            fig=px.area(fw,x="event_date",y="ev",color="event_action",title="Firewall Actions",color_discrete_map={"drop":"#f85149","accept":"#3fb950","reject":"#ffa657"}); show(fig,h=260)
        ...
        if not bl.empty:
            fig=px.treemap(bl,path=["src_ip"],values="block_count",color="targeted_ports",title="Top Blocked IPs",color_continuous_scale="Reds"); show(fig,h=300)
...
# =============================================================================
    ...
    with c1:
        if not ids.empty:
            ...
            fig.update_layout(title="IDS Alerts",legend=dict(orientation="h",y=-0.2),height=280,**PT); st.plotly_chart(fig,use_container_width=True)
    with c2:
        if not ti.empty:
            fig=px.area(ti,x="event_date",y="indicator_count",title="Threat Intel Indicators",color_discrete_sequence=["#d2a8ff"]); show(fig,h=280)
    sigs=qdf(f"SELECT signature,SUM(alert_count) AS cnt FROM mart_ids_alerts WHERE {DR} GROUP BY 1 ORDER BY 2 DESC LIMIT 12")
    if not sigs.empty:
        fig=px.bar(sigs,x="cnt",y="signature",orientation="h",title="Top IDS Signatures",color="cnt",color_continuous_scale="Reds"); show(fig,h=360)
    dns_t=qdf(f"SELECT * FROM mart_dns_summary WHERE possible_dns_tunnel OR possible_dga ORDER BY txt_queries DESC LIMIT {TOP_N}")
    if not dns_t.empty:
        st.markdown("#### DNS Tunnelling / DGA Suspects"); st.dataframe(dns_t,use_container_width=True)
...
# =============================================================================
    ...
    with t1:
        ...
        if not auth.empty:
            ...
            fig.update_layout(title="Login Trends",barmode="group",legend=dict(orientation="h",y=-0.2),height=320,**PT); st.plotly_chart(fig,use_container_width=True)
    with t2:
        ...
        if not bf.empty:
            fig=px.scatter(bf,x="failure_count",y="username",size="failure_count",color="severity",
                ...
                title="Brute Force Suspects",hover_data=["src_ip"]); show(fig,h=340)
        else: st.info("No brute-force data.")
    with t3:
        ah=qdf(f"SELECT username,login_hour,COUNT(*) AS cnt FROM mart_afterhours_logins GROUP BY 1,2 ORDER BY 3 DESC LIMIT {TOP_N}")
        if not ah.empty:
            fig=px.density_heatmap(ah,x="login_hour",y="username",z="cnt",title="After-Hours Login Heatmap",color_continuous_scale="Reds"); show(fig,h=320)
...
# =============================================================================
    ...
    with t1:
        ...
        if not risk.empty:
            ...
            with c1:
                fig=px.pie(risk,values="cnt",names="risk_classification",title="Process Risk",hole=0.45,
                    color="risk_classification",color_discrete_map={"MALICIOUS":"#f85149","SUSPICIOUS":"#ffa657","NORMAL":"#3fb950"}); show(fig,h=260)
            with c2:
                ...
                if not tp.empty:
                    fig=px.bar(tp,x="cnt",y="process_image",orientation="h",color="risk_classification",
                        color_discrete_map={"MALICIOUS":"#f85149","SUSPICIOUS":"#ffa657"},title="Risky Processes"); show(fig,h=260)
        ...
        st.markdown("#### Suspicious Processes"); st.dataframe(sp,use_container_width=True,height=220)
    with t2:
        ...
        if not ps.empty:
            fig=px.funnel(ps,x="cnt",y="risk_level",title="PowerShell Risk Funnel",color="risk_level",
                color_discrete_map={"CRITICAL":"#f85149","HIGH":"#ffa657","INFORMATIONAL":"#58a6ff"}); show(fig,h=260)
    ...
    with t3:
        ...
        if not sl.empty:
            fig=px.bar(sl,x="process_name",y="cnt",color="severity",
                color_discrete_map={"err":"#f85149","warning":"#ffa657","info":"#58a6ff","debug":"#8b949e"},title="Linux Syslog by Severity")
            fig.update_layout(xaxis_tickangle=-45); show(fig,h=280)
        ...
        if not ssh.empty:
            ...
            fig.update_layout(title="SSH Activity",barmode="stack",legend=dict(orientation="h",y=-0.2)); show(fig,h=260)
...
# =============================================================================
    ...
    if not atk.empty:
        c1,c2=st.columns(2)
        with c1:
            fig=px.bar(atk,x="attack_type",y="event_count",color="attack_type",title="Events by Attack Type",color_discrete_sequence=px.colors.qualitative.Bold); show(fig,h=260)
        with c2:
            fig=px.scatter(atk,x="duration_minutes",y="hosts_affected",size="event_count",color="attack_type",title="Duration vs Hosts",color_discrete_sequence=px.colors.qualitative.Bold); show(fig,h=260)
    if not mit.empty:
        agg=mit.groupby(["kill_chain_phase","attack_type"])["event_count"].sum().reset_index()
        fig=px.density_heatmap(agg,x="attack_type",y="kill_chain_phase",z="event_count",title="Kill Chain Phase x Attack Type",color_continuous_scale="RdYlGn_r"); show(fig,h=340)
        fig=px.sunburst(mit,path=["kill_chain_phase","mitre_technique"],values="event_count",title="MITRE ATT&CK Kill Chain",color="event_count",color_continuous_scale="Reds"); show(fig,h=460)
    ...
    if not tl.empty:
        fig=px.scatter(tl,x="event_time",y="attack_type",color="mitre_technique",symbol="hostname",title="Attack Event Timeline"); show(fig,h=300)
...
# =============================================================================
    ...
    with ca:
        fig=px.histogram(anom,x="z_score",color="anomaly_label",title="Z-Score Distribution",
            color_discrete_map={"Anomalous":"#f85149","Elevated":"#ffa657","Normal":"#3fb950","Low Activity":"#8b949e"},nbins=40); show(fig,h=280)
    with cb:
        ...
        if not top.empty:
            ...
            fig.update_layout(xaxis_tickangle=-45); show(fig,h=280)
    fig=px.scatter(anom,x="mean",y="event_count",color="anomaly_label",size="z_score",hover_data=["hostname"],title="Expected vs Actual Events",
        color_discrete_map={"Anomalous":"#f85149","Elevated":"#ffa657","Normal":"#3fb950","Low Activity":"#8b949e"}); show(fig,h=360)
    st.dataframe(anom[anom["anomaly_label"]=="Anomalous"].nlargest(20,"z_score"),use_container_width=True,height=240)
    ...
    if not http.empty:
        ...
        with c1:
            ...
            fig=px.pie(agg,values="request_count",names="group",title="HTTP Response Groups",hole=0.4,
                color="group",color_discrete_map={"2xx OK":"#3fb950","3xx Redirect":"#58a6ff","4xx Client Error":"#ffa657","5xx Server Error":"#f85149"}); show(fig,h=260)
        with c2:
            ma=http.groupby("method")["request_count"].sum().reset_index()
            fig=px.bar(ma,x="method",y="request_count",title="HTTP Methods",color="request_count",color_continuous_scale="Blues"); show(fig,h=260)
    if not err.empty:
        fig=go.Figure()
        ...
        fig.update_layout(title="Hourly Requests & Errors",legend=dict(orientation="h",y=-0.2)); show(fig,h=280)
...
# =============================================================================
    ...
    if _HAS_HELPERS:
        ...
        sql = st.text_area("Direct SQL:", "SELECT * FROM mart_kpi_daily ORDER BY event_date LIMIT 10")
        if st.button("Run"):
...
# =============================================================================
    ...
    if _HAS_HELPERS:
        ...
        for r in ["Enforce MFA on all privileged accounts",
                  "Review firewall DROP rules for persistent offenders",
                  "Investigate anomalous hosts with Z-score > 3",
                  "Tune IDS signatures to reduce false positives",
