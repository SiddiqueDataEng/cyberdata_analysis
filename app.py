...
- Cloud (Streamlit Community): synthetic demo data, LIMIT on all queries
- Local: full DuckDB warehouse, no row limits
"""
...
.kpi-card{background:linear-gradient(135deg,#161b22,#21262d);border:1px solid
 #30363d;border-radius:12px;padding:20px 14px;text-align:center;margin:4px 2px}
...
# ── Detect environment ────────────────────────────────────────────────────────
IS_CLOUD = not os.path.exists("/mount/src") is False or os.environ.get("STREAMLIT_SHARING_MODE") == "1"
try:
    from config import DB_PATH
    IS_CLOUD = not os.path.exists(DB_PATH)
except Exception:
    DB_PATH = ""
    IS_CLOUD = True

# Cloud: limit rows per query to save CPU; local: no limit
ROW_LIMIT = "LIMIT 1000" if IS_CLOUD else ""
TOP_LIMIT = "LIMIT 50"   if IS_CLOUD else "LIMIT 200"


# ─────────────────────────────────────────────────────────────────────────────
# DEMO WAREHOUSE (cloud only)
# ─────────────────────────────────────────────────────────────────────────────
def _build_demo_warehouse():
    ...
    def mk(tbl, df):
        ...
        con.execute(f"CREATE TABLE {tbl} AS SELECT * FROM {vname}")
        con.unregister(vname)
    ...
    mk("mart_ids_alerts", pd.DataFrame({
        "event_date": dates,
        "signature": rng.choice(["ET SCAN Port Scan", "ET MALWARE Trojan", "ET EXPLOIT Buffer Overflow", "ET POLICY Suspicious UA"], n).tolist(),
    ...
    mk("mart_suspicious_processes", pd.DataFrame({
        ...
        "process_image": rng.choice(["C:\\powershell.exe", "C:\\mimikatz.exe", "C:\\cmd.exe"], 30).tolist(),
        "command_line": rng.choice(["powershell -enc ABC", "mimikatz.exe", "cmd /c whoami"], 30).tolist(),
        "parent_image": ["C:\\svchost.exe"] * 30,
    ...
    mk("mart_risky_powershell", pd.DataFrame({
        ...
        "script_block": rng.choice(["IEX DownloadString(...)", "Get-Process", "New-LocalUser -Name backdoor"], 20).tolist(),
    ...
    n_fe = 500
    atk_pool = rng.choice(["apt", "ransomware", "data_exfil", "cred_theft", "", "", ""], n_fe).tolist()
    mit_pool = rng.choice(["T1059.001", "T1021", "T1041", "T1003", "T1566", "", ""], n_fe).tolist()
    mk("fact_events", pd.DataFrame({
        ...
        "attack_type": [x if x.strip() else None for x in atk_pool],
        "mitre_technique": [x if x.strip() else None for x in mit_pool],
...
def qlim(sql: str) -> pd.DataFrame:
    """Apply row limit on cloud, pass-through locally."""
    if IS_CLOUD and "LIMIT" not in sql.upper():
        sql = sql.rstrip().rstrip(";") + f" {ROW_LIMIT}"
    return qdf(sql)
...
with st.sidebar:
    ...
    if IS_CLOUD:
        st.info("Running on demo data (cloud mode)")
    st.divider()
...
if page == "Executive Summary":
    ...
    for col, icon, val, lbl in [
        (c1, "Events", fmt_number(kpi["te"].iloc[0]), "Total Events"),
        (c2, "Alerts", fmt_number(kpi["ia"].iloc[0]), "IDS Alerts"),
        (c3, "Attacks", fmt_number(kpi["ae"].iloc[0]), "Attack Events"),
        (c4, "AuthFail", fmt_number(kpi["af"].iloc[0]), "Auth Failures"),
        (c5, "FWBlock", fmt_number(kpi["fd"].iloc[0]), "FW Blocks"),
    ]:
    ...
    with col_l:
        tr = qdf(f"""SELECT event_date, total_events, ids_alerts, attack_events, auth_failures
            FROM mart_kpi_daily WHERE {DRANGE} ORDER BY event_date {ROW_LIMIT}""")
        if not tr.empty:
            fig = go.Figure()
...
# =============================================================================
    ...
    kpi_df = qdf(f"SELECT * FROM mart_kpi_daily WHERE {DRANGE} ORDER BY event_date {ROW_LIMIT}")
    if kpi_df.empty:
    ...
    rsk = qdf(f"SELECT * FROM mart_risk_score_daily WHERE {DRANGE} ORDER BY event_date {ROW_LIMIT}")
    if not rsk.empty:
...
# =============================================================================
    ...
    with tab1:
        ext = qdf(f"""SELECT CAST(DATE_TRUNC('day',hour_window) AS DATE) AS dt,
            SUM(inbound_external) AS inbound, SUM(outbound_external) AS outbound
            FROM mart_external_traffic
            WHERE CAST(DATE_TRUNC('day',hour_window) AS DATE) BETWEEN '{d_start}' AND '{d_end}'
            GROUP BY 1 ORDER BY 1 {ROW_LIMIT}""")
        if not ext.empty:
            fig = go.Figure()
    ...
    with tab2:
        tk = qdf(f"SELECT src_ip, connection_count, total_bytes, unique_destinations, unique_dest_ports FROM mart_top_talkers ORDER BY connection_count DESC {TOP_LIMIT}")
        if not tk.empty:
    ...
    with tab3:
        sc = qdf(f"SELECT src_ip, SUM(unique_ports_targeted) AS ports, SUM(total_attempts) AS attempts, scan_risk_level FROM mart_port_scan_suspects GROUP BY 1,4 ORDER BY 2 DESC {TOP_LIMIT}")
        if not sc.empty:
    ...
    with tab4:
        fw = qdf(f"SELECT event_date, event_action, SUM(event_count) AS events FROM mart_firewall_summary WHERE {DRANGE} GROUP BY 1,2 ORDER BY 1 {ROW_LIMIT}")
        if not fw.empty:
            ...
            show(fig, h=300)
        bl = qdf(f"SELECT src_ip, block_count, targeted_ports, targeted_hosts FROM mart_fw_blocked_sources ORDER BY block_count DESC {TOP_LIMIT}")
        if not bl.empty:
...
# =============================================================================
    ...
    ids = qdf(f"SELECT event_date, SUM(alert_count) AS alerts, SUM(unique_sources) AS sources FROM mart_ids_alerts WHERE {DRANGE} GROUP BY 1 ORDER BY 1 {ROW_LIMIT}")
    ti = qdf(f"SELECT * FROM mart_threat_intel WHERE {DRANGE} ORDER BY event_date {ROW_LIMIT}")
    c1, c2 = st.columns(2)
    with c1:
        if not ids.empty:
    ...
    if not sigs.empty:
        ...
        show(fig, h=420)
    dns_t = qdf(f"SELECT * FROM mart_dns_summary WHERE possible_dns_tunnel OR possible_dga ORDER BY txt_queries DESC {TOP_LIMIT}")
    if not dns_t.empty:
...
# =============================================================================
    ...
    with tab1:
        auth = qdf(f"SELECT event_date, SUM(failures) AS failures, SUM(successes) AS successes, ROUND(AVG(failure_rate_pct),2) AS rate FROM mart_auth_trend WHERE {DRANGE} GROUP BY 1 ORDER BY 1 {ROW_LIMIT}")
        if not auth.empty:
            fig = make_subplots(specs=[[{"secondary_y": True}]])
            ...
            st.plotly_chart(fig, use_container_width=True)
    with tab2:
        bf = qdf(f"SELECT src_ip, username, failure_count, severity, targets FROM mart_brute_force ORDER BY failure_count DESC {TOP_LIMIT}")
        if not bf.empty:
    ...
    with tab3:
        ah = qdf(f"SELECT username, login_hour, COUNT(*) AS cnt FROM mart_afterhours_logins GROUP BY 1,2 ORDER BY 3 DESC {TOP_LIMIT}")
        if not ah.empty:
...
# =============================================================================
    ...
    with tab1:
        ...
        if not risk.empty:
            ...
            with c1:
                ...
                show(fig, h=300)
            with c2:
                tp = qdf(f"SELECT process_image, SUM(execution_count) AS cnt, risk_classification FROM mart_suspicious_processes WHERE risk_classification!='NORMAL' GROUP BY 1,3 ORDER BY 2 DESC {TOP_LIMIT}")
                if not tp.empty:
                    ...
                    show(fig, h=300)
        sp = qdf(f"SELECT event_date, hostname, username, process_image, LEFT(command_line,80) AS cmd, risk_classification FROM mart_suspicious_processes WHERE risk_classification IN ('MALICIOUS','SUSPICIOUS') ORDER BY risk_classification, event_date DESC {TOP_LIMIT}")
    ...
    with tab2:
        ...
        if not ps.empty:
            ...
            show(fig, h=300)
        psd = qdf(f"SELECT event_date, hostname, username, LEFT(script_block,120) AS preview, risk_level FROM mart_risky_powershell WHERE risk_level IN ('CRITICAL','HIGH') ORDER BY risk_level, event_date DESC {TOP_LIMIT}")
    ...
    with tab3:
        ...
        if not sl.empty:
            ...
            fig.update_layout(xaxis_tickangle=-45)
            show(fig, h=340)
        ssh = qdf(f"SELECT * FROM mart_ssh_attempts WHERE {DRANGE} ORDER BY event_date {ROW_LIMIT}")
        if not ssh.empty:
            fig = go.Figure()
...
# =============================================================================
    ...
    if not mit.empty:
        ...
        fig = px.density_heatmap(agg, x="attack_type", y="kill_chain_phase", z="event_count",
            title="Kill Chain Phase x Attack Type", color_continuous_scale="RdYlGn_r")
        show(fig, h=400)
        fig = px.sunburst(mit, path=["kill_chain_phase", "mitre_technique"], values="event_count",
            title="MITRE ATT&CK - Kill Chain to Technique", color="event_count", color_continuous_scale="Reds")
        show(fig, h=500)
    tl = qdf(f"SELECT event_time, attack_type, mitre_technique, hostname FROM fact_events WHERE attack_type IS NOT NULL AND attack_type!='' ORDER BY event_time {TOP_LIMIT}")
    if not tl.empty:
...
# =============================================================================
    ...
    anom = qdf(f"SELECT * FROM mart_host_anomaly ORDER BY z_score DESC {ROW_LIMIT}")
    if anom.empty:
    ...
    show(fig, h=400)
    st.dataframe(anom[anom["anomaly_label"] == "Anomalous"].nlargest(30, "z_score"), use_container_width=True, height=300)
...
# =============================================================================
    ...
    http = qdf(f"SELECT * FROM mart_http_status WHERE {DRANGE} ORDER BY event_date {ROW_LIMIT}")
    err = qdf(f"SELECT * FROM mart_web_error_trend WHERE CAST(DATE_TRUNC('day',hour_window) AS DATE) BETWEEN '{d_start}' AND '{d_end}' ORDER BY hour_window {ROW_LIMIT}")
    if not http.empty:
    ...
    if not err.empty:
        ...
        fig.update_layout(title="Hourly Request Volume & Errors", legend=dict(orientation="h", y=-0.2))
        show(fig, h=340)
